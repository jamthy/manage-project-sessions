...
flowchart LR
    ...
    B --> D{"当前边界是否仍然完整？"}
    D -->|是| E["继续当前任务，通常静默执行"]
    D -->|出现独立目标、新阶段或跨专题协作| F{"已建立项目总控？"}
    F -->|是| I["向项目总控返回证据，由总控决定下一专题"]
    F -->|否| J["按当前边界建议创建专题或继续"]
...
当项目已经建立“项目总控 + 专题任务”结构后，项目总控是专题编排的唯一入口。专题任务在以下真实边界出现时，只向总控汇报并停止自行扩展范围：当前阶段或停止条件完成、需要另一个专题的结果或决策、需要切换语义所有者或进入新阶段，以及用户在专题内要求创建或切换其他专题。

汇报只包含总控决策所需的最小关闭证据：实际状态、变更文件或产物、验证结果、未解决和未验证事项、一个下一步建议；如涉及任务操作，还要保留用户的精确请求及可核对的授权来源。专题不得自行创建、交接、恢复、切换或指挥其他专题，也不得把跨专题依赖变成专题之间的直接编排。

总控负责判断下一边界、协调依赖并选择下一个专题，但这不是超级权限。创建、交接、恢复、切换、进入、重命名、归档或删除任务，仍然需要用户对具体动作的明确授权；总控只能依据宿主可核对的用户原始请求，不能把专题的转述当作授权。如果无法从委派、宿主元数据、已验证交接或用户指示中确认总控的精确任务标识，专题会停在当前边界并请求用户建立或恢复总控，不会根据标题、仓库或最近任务猜测 ID。

这套规则不要求所有项目都建立总控。短项目或单一所有者项目可以继续在一个边界清楚的任务中完成；普通专题内的编辑、测试、澄清和可恢复失败也不会触发逐步汇报。
...
- 创建、交接、恢复、切换、进入、重命名、归档或删除 Codex 任务需要用户对具体动作明确授权。
- 已建立总控/专题结构时，专题只在完成、跨专题依赖或真实边界事件向总控汇报；只有总控负责专题编排，但总控仍不能绕过逐项用户授权。
